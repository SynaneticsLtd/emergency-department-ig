...
* characteristic[=].code.text = "Organization.identifier.where(value='RET34')"
* characteristic[=].valueCodeableConcept.coding[0].code = $ods-code#RET34
* characteristic[=].exclude = false

// Members

* member[0].entity = Reference(Patient/Patient-EoinTest)
* member[=].period.start = "2000-01-01"
* member[+].entity = Reference(Patient/Patient-FredTest)
* member[=].period.start = "2000-01-01"
